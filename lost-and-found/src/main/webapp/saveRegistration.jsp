...
if (captcha.trim().equals(sesCap.trim())){
	...
	String reg_cmd="insert into registration (name, email, mobno, gender, dob,  address, terms, picture) values('"+name+"','"+email+"','"+mobno+"','"+gender+"','"+dob+"','"+address+"','"+terms+"','"+fname+"')";
	...
	if(log_status && reg_status){
		...
		out.print("<script>alert('Registration failed, please try again later.');window.location.href='registration.jsp'</script>");
	}
}else{
	out.print("<script>alert('Invalid Captcha code, please try againg later.');window.location.href='registration.jsp'</script>");
}
